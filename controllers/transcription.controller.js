...
exports.createTranscription = async (req, res) => {
  try {
    const { audioFileId, userId, transcriptionText, language, status, errorDetails } = req.body;
    ...
    res.status(201).json({ message: 'Transcription created successfully', transcription: newTranscription });
  } catch (err) {
...
exports.getUserTranscriptions = async (req, res) => {
  try {
    const transcriptions = await Transcription.find({ userId: req.params.userId });
